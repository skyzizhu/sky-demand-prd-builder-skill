...
description: 负责将用户/业务提出的需求转化为结构化的产品需求与功能设计，包括标准化的 PRD 草稿、数据/埋点与监控方案草稿，以及灰度发布和 A/B 测试等实验设计要点，并标出需要人工补充的关键项，为后续评审与导出 PDF 做结构准备。既支持基于 sky-demand-evaluator 的真需求结果自动生成，也支持用户直接输入产品需求描述后生成 PRD（此时自动增加更多 TODO 以提示先完善或评估）。
---
...
- **输出格式要求**：元信息必须使用 Markdown 表格，三列为「字段 / 内容 / 备注」。  
  - 「内容」列填写具体值或 `TODO(...)`；  
  - 「备注」列用于补充说明，不需要可留空。  
...
在「关键用户路径」基础上，补充一个更贴近交互层的节点链路，要求：

- 以编号步骤列出，覆盖入口 → 素材/数据选择或输入 → 参数配置（基础/高级） → 结果展示与操作 → 错误处理；  
- 结果操作至少包含保存/分享/继续操作等典型动作（按需求适配）；  
- 错误处理需明确呈现方式（失败页/弹框/提示条）与重试路径；  
- 如某类节点不适用，可删减，但需在 TODO 中说明原因。  
...
为了让使用者更容易理解本 skill 与 `sky-demand-evaluator` 的配合方式，可以参考以下串联流程：
...
3. 当结论为真需求且评分>50% 时，将上述结构化结果作为本 skill 的输入：  
   - 形式 A：直接把 `sky-demand-evaluator` 的完整输出交给本 skill 继续处理；  
